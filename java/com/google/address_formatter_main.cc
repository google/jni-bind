...
#include "address_formatter.h"  // NOLINT
#include "jni_bind.h"
...
using address::formatter::AddressFormatter;
using jni::ThreadGuard;

static constexpr int32_t num_threads = 10;
static constexpr int32_t num_iterations = 1000;
...
std::optional<void *> LoadLibrary(std::string_view lib_path) {
...
void PrintAddressWorkerThread(
    std::mutex *condition_variable_mutex,
    std::condition_variable *work_done_condition_variable,
    int *threads_remaining, jni::JvmRef<jni::kDefaultJvm> *jvm_ref,
    AddressFormatter *address_formatter, int thread_id, int num_iterations) {
  ThreadGuard thread_guard = jvm_ref->BuildThreadGuard();
  for (int i = 0; i < num_iterations; ++i) {
    // This output appears to be tearing, but it's not crashing which is the
    // important part.
    std::cout << "thread id: " << thread_id << "||"
              << address_formatter->Format() << std::endl;
  }

  std::unique_lock<std::mutex> lk(*condition_variable_mutex);
  (*threads_remaining)--;
  work_done_condition_variable->notify_one();
}

void RunIterationsToCompletion(jni::JvmRef<jni::kDefaultJvm> *jvm_ref) {
  const int kNumThreads = num_threads;
  const int kNumIterations = num_iterations;

  AddressFormatter address_formatter;
  std::mutex condition_variable_mutex;
  std::condition_variable work_done_condition_variable;

  // Setup threads.
  // See https://en.cppreference.com/w/cpp/thread/condition_variable.
  int threads_remaining = kNumThreads;
  std::vector<std::thread> threads;
  threads.reserve(kNumThreads);
  for (int i = 0; i < kNumThreads; ++i) {
    threads.emplace_back(PrintAddressWorkerThread, &condition_variable_mutex,
                         &work_done_condition_variable, &threads_remaining,
                         jvm_ref, &address_formatter, i, kNumIterations);
  }
  ...
  // Wait until all threads signal completion.
  {
    std::unique_lock<std::mutex> lock(condition_variable_mutex);
    work_done_condition_variable.wait(lock,
                                      [&] { return threads_remaining == 0; });
  }
}
